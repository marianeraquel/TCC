...
public class GeraDados extends Configured implements Tool {
    ...
    protected void geraDadosNormal(String path, int potencia, int cenarios, int arquivos) throws IOException {
        JobConf conf = new JobConf();

        long num_dados = (long) Math.pow(10.0, potencia);
        FileSystem fs = FileSystem.get(conf);

        for (int j = 1; j <= cenarios; j++) {
            ...
            for (int k = 1; k <= arquivos; k++) {
                ...
                SequenceFile.Writer out = new SequenceFile.Writer(fs, conf, arq, FloatWritable.class, Text.class);

                // gera os dados e grava os arquivos
                long dados_por_divisao = num_dados / arquivos;
                for (int i = 0; i < dados_por_divisao; i++) {
                    float valores_inteiros = (float) Math.abs(RandomGenerator.gaussian(5.0, 1.0));
                    out.append(new FloatWritable(valores_inteiros), new Text(""));
                }
                out.close();
            }
        }
    ...
    protected void geraDadosPareto(String path, int potencia, int cenarios, int arquivos) throws IOException {
        JobConf conf = new JobConf();
        ...
        for (int j = 1; j <= cenarios; j++) {
            ...
            for (int k = 1; k <= arquivos; k++) {
                ...
                long dados_por_divisao = num_dados / arquivos;
                for (int i = 0; i < dados_por_divisao; i++) {
                    float valores_inteiros = (float) Math.abs(RandomGenerator.pareto(0.9));
                    out.append(new FloatWritable(valores_inteiros), new Text(""));
                }
                out.close();
            }
    ...
    public int run(String[] args) throws IOException {
        ...
        if (args.length != 5) {
            System.err.printf("Uso: %s [opcoes genericas] <dir> <num_dados><num_arq><dist>\n", getClass().getSimpleName());
        ...
        // Chama função para gerar dados
        if (args[4].equals("3")) {
            System.out.println("Criando distribuição PARETO");
            geraDadosPareto(args[0], Integer.parseInt(args[1]), Integer.parseInt(args[2]), Integer.parseInt(args[3]));
        } else if (args[4].equals("2")) {
            System.out.println("Criando distribuição NORMAL");
            geraDadosNormal(args[0], Integer.parseInt(args[1]), Integer.parseInt(args[2]), Integer.parseInt(args[3]));
        } else {
            System.out.println("Criando distribuição UNIFORME");
