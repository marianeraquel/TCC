...
public class OrdenacaoPorAmostragem extends Configured implements Tool {
    ...
    *//**
    ...
    public int run(String[] args) throws Exception {
        ...
        System.out.println(" OK. " + diretorio_pai.getWorkingDirectory());
        for (FileStatus diretorio_filho : diretorio_pai.listStatus(new Path(dir))) {
            System.out.println(" dir filho " + diretorio_filho.getPath());
...
//                System.out.println(" dir result " + dir_resultados + " " + nome_arq_saida_1);
                ...
                printWriter1.print("# Freq Amostras    Tempo(ms) Particoes\n");
                printWriter2.print("# Freq Amostras    Tempo(ms) Particoes\n");
                ...
                for (double freq = freq_min; freq <= freq_max; freq += freq_passo) {
                    for (int num_amostras = num_amostras_min; num_amostras <= num_amostras_max; num_amostras += num_amostras_passo) {
                        ...
                        for (int i = 1; i <= max_execucoes; i++) {
                            ...
                            InputSampler.Sampler<FloatWritable, Text> sampler = new InputSampler.RandomSampler<FloatWritable, Text>(freq, num_amostras, max_particoes);

                            // seta o caminho para criar o arquivo binario das particoes
                            Path input = FileInputFormat.getInputPaths(conf)[0];
                            input = input.makeQualified(input.getFileSystem(conf));
//                            System.out.println(" path. " + input.getName());
                            ...
                            for (FileStatus arq_saida : diretorio_saida.listStatus(new Path(path_saida))) {
                                ...
                                if (!arq_saida.isDir() && arq_saida.getPath().toString().contains("part")) {
                                    System.out.println(j + "  " + arq_saida.getPath().toString());
