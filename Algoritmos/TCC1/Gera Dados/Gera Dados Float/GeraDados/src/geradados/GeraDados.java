...
public class GeraDados extends Configured implements Tool {
    ...
    public static class UniformMapper extends MapReduceBase
        ...
        public void map(LongWritable row, NullWritable ignored,
            ...
            // Gera chave
            key.set(Math.abs(RandomGenerator.uniform(0)));
            // Não tem dados valor
            value.clear();
    ...
    public static class NormalMapper extends MapReduceBase
            implements Mapper<LongWritable, NullWritable, FloatWritable, Text> {

        private FloatWritable key = new FloatWritable();
        private Text value = new Text();
        ...
        public void map(LongWritable row, NullWritable ignored,
            ...
            // Gera chave
            key.set(Math.abs((float) RandomGenerator.gaussian(5.0, 1.0)));
            // Não tem dados valor
            value.clear();
            output.collect(key, value);
        }
    ...
    public static class ParetoMapper extends MapReduceBase
        ...
        public void map(LongWritable row, NullWritable ignored,
            ...
            // Gera chave
            key.set((float) RandomGenerator.pareto(0.9));
            // Não tem dados valor
            value.clear();
            output.collect(key, value);
        }
