        ...
        public class TotalOrderPartitioner<K extends WritableComparable, V>
            ...
            public void configure(JobConf job) {
                try {
                    ...
                    final FileSystem fs = (DEFAULT_PATH.equals(parts)) ? FileSystem
                            .getLocal(job) // assume in DistributedCache
                            : partFile.getFileSystem(job);

                    Class<K> keyClass = (Class<K>) job.getMapOutputKeyClass();
                    K[] splitPoints = readPartitions(fs, partFile, keyClass,
                            job);
                    ...
                    for (int i = 0; i < splitPoints.length - 1; ++i) {
                        if (comparator.compare(splitPoints[i],
                                splitPoints[i + 1]) >= 0) {
                            throw new IOException(
                                    "Split points are out of order");
                        }
                    }
                    boolean natOrder = job.getBoolean(
                            "total.order.partitioner.natural.order", true);
                    if (natOrder
                            && BinaryComparable.class
                                    .isAssignableFrom(keyClass)) {
                        partitions = buildTrie(
                                (BinaryComparable[]) splitPoints,
                                0,
                                splitPoints.length,
                                new byte[0],
                                job
                                        .getInt(
                                                "total.order.partitioner.max.trie.depth",
                                                2));
                    } else {
            ...
            class InnerTrieNode extends TrieNode {
                ...
                public int findPartition(BinaryComparable key) {
                    ...
                    return child[0xFF & key.getBytes()[level]]
                            .findPartition(key);
                }
            }
            ...
            class LeafTrieNode extends TrieNode {
                ...
                LeafTrieNode(int level, BinaryComparable[] splitPoints,
                        int lower, int upper) {
                    super (level);
                    this .lower = lower;
                    this .upper = upper;
                ...
                public int findPartition(BinaryComparable key) {
                    final int pos = Arrays.binarySearch(splitPoints, lower,
                            upper, key) + 1;
                    return (pos < 0) ? -pos : pos;
                }
            }
            ...
            private K[] readPartitions(FileSystem fs, Path p,
                    Class<K> keyClass, JobConf job) throws IOException {
                ...
                reader.close();
                return parts.toArray((K[]) Array.newInstance(keyClass, parts
                        .size()));
            }
            ...
            private TrieNode buildTrie(BinaryComparable[] splits, int lower,
                ...
                for (int ch = 0; ch < 255; ++ch) {
                    ...
                    trial[depth] = (byte) ch;
                    result.child[0xFF & ch] = buildTrie(splits, lower,
                            currentBound, trial, maxDepth);
                }
                // pick up the rest
                trial[depth] = 127;
                result.child[255] = buildTrie(splits, currentBound, upper,
                        trial, maxDepth);
                return result;
            }
        }
